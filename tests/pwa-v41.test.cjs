...
test('4.1 提供可安裝 PWA manifest 與手機圖示',()=>{
  const manifest=JSON.parse(fs.readFileSync(path.join(root,'manifest.webmanifest'),'utf8'));
  assert.equal(manifest.name,'稽查助手4.1.1');
...
test('4.1 首頁載入 manifest、PWA 啟動程式及手機 viewport',()=>{
...
test('4.1 service worker App Shell 僅快取本機現有資源',()=>{
  const sw=fs.readFileSync(path.join(root,'service-worker.js'),'utf8');
  assert.match(sw,/inspection-assistant-4\.1\.1-pp-7f3c9a21/);
