...
test('4.7 手機聚焦只屬呈現層，不改 Water Core 規則結果',async()=>{const {root,config}=await loaded();const t=config.templates.find(x=>x.id==='water-main');const input={waterBehaviorDate:'2026-09-11',waterInspectionDate:'2026-09-11',waterSubjectType:'business',waterSubjectConfirmed:'yes',waterMatterType:'wastewater',waterWastewaterStatus:'yes',waterActualDischarge:'yes',waterDestination:'surfaceWater',waterSurfaceWaterConfirmed:'unknown',waterInvestigationComplete:'yes'};const n=root.DraftEngine.normalize(t,input);assert.equal(n.waterRuleStatus,'insufficient');assert.match(n.waterFinalConclusionText,/C｜尚有要件待確認/);});
