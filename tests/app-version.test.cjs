...
test('4.2.1 應用程式版本集中管理並由首頁載入',()=>{
  const context=vm.createContext({window:{}});
  vm.runInContext(fs.readFileSync(path.join(__dirname,'../data/app-meta.js'),'utf8'),context);
  assert.equal(context.window.INSPECTION_APP_META.version,'4.2.1');
  assert.equal(context.window.INSPECTION_APP_META.label,'稽查助手4.2.1');
