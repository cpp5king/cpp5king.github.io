...
test('依清單載入現有範本，不需在首頁列出每個 script', async () => {
  ...
  assert.deepEqual(requested, ['restaurant-odor.js', 'restaurant-odor-sampling-pending.js', 'noise-case.js', 'noise-main.js', 'noise-neighbor.js', 'water-field.js', 'water-main.js']);
  assert.equal(config.templates.length, 7); assert.equal(config.templates[0].title, '現有範本');
...
test('新增純測試資料只需登記清單，載入器可載入多模板', async () => {
  ...
  assert.equal(config.templates.length, 8);
});
