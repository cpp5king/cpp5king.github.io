...
test('正確載入大類與類型；空氣污染、水污染與噪音開放', async () => {
  ...
  assert.equal(config.caseTypes.length, 4); assert.equal(config.caseTypes[0].title, '餐飲異味');
});
test('依清單載入現有範本，不需在首頁列出每個 script', async () => {
  ...
  assert.deepEqual(requested, ['restaurant-odor.js', 'restaurant-odor-sampling-pending.js', 'noise-case.js', 'noise-main.js', 'noise-neighbor.js', 'water-main.js']);
