...
test('5.2.6 test candidate metadata keeps provenance and test build marker',()=>{
  const context=vm.createContext({window:{}});
  vm.runInContext(read('data/app-meta.js'),context);
  const meta=context.window.INSPECTION_APP_META;
  assert.equal(meta.version,'5.2.6');
  assert.equal(meta.label,'稽查助手5.2.6');
  assert.equal(meta.build,'test-5.2.6-noise-refactor-1');
  ...
  assert.match(read('service-worker.js'),/inspection-assistant-5\.2\.6-pp-7f3c9a21-/);
  assert.doesNotMatch(read('service-worker.js'),/inspection-assistant-test-5\.2\.6/);
  assert.match(read('index.html'),/theme-color" content="#57534e"/);
  assert.match(read('manifest.webmanifest'),/"background_color": "#efede8"/);
  assert.match(read('manifest.webmanifest'),/"theme_color": "#57534e"/);
});
