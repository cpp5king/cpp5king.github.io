...
const supersededCases = [
  ...
  '同層兩模板點選直接進表單，往返不殘留輸入與草稿',
  '水污染 V2 已接入稽查助手主入口，噪音入口仍保留',
  '疑似來源可直接進行對象查核並可回寫來源判斷',
  '5.0 RC3 gives four modules separate theme identities',
  '選擇採樣樣態後實際渲染日期、分鐘時間、袋數、公司欄位且不提供程序控制欄位',
  '勾選才顯示公司名稱，取消勾選清除名稱',
  '兩個獨立模板同屬空氣污染，原模板保留',
  '切換回原樣態再回採樣時，輸入與手動草稿皆清空',
  '正確載入大類與類型；5.0 四大模組均開放',
  '依清單載入現有範本，不需在首頁列出每個 script',
  'law navigation is available as a separate onsite layer',
  '5.0 shell loads original Waste V0.1.7 runtime in dependency order and caches it offline',
  '情境1｜來源不明：追水可停在疑似來源，確認來源後再進對象查核，不提前產生法規方向'
];
