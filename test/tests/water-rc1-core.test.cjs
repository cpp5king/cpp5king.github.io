...
async function norm(input){
  ...
  return plain(env.root.DraftEngine.normalize(t,{waterBehaviorDate:'2026-09-11',...input}));
}
...
test('4.0-rc1 §13 指定事業於設立變更前查無核准水措計畫可形成成立方向',async()=>{
  ...
  assert.match(f.waterArticle13Text,/構成要件事實已完整/);
...
test('4.0-rc1 §18 未進子法時只提示待查，已確認具體義務且不符才成立',async()=>{
  ...
  assert.match(pending.waterArticle18Text,/授權型水措義務|水措管理規定/);
  ...
  assert.match(hit.waterArticle18Text,/構成要件事實已完整/);
});
...
test('4.0-rc1 沒有向外排放仍可追到貯留並進§20',async()=>{
  ...
  assert.match(f.waterArticle20Text,/構成要件事實已完整/);
});
...
test('4.0-rc1 §22與§35分離，資料不一致不直接等於明知不實',async()=>{
  ...
  assert.match(f.waterArticle22Text,/目前不支持/);
  assert.match(f.waterArticle35Text,/尚有要件待確認/);
...
test('4.0-rc1 §35須有明知證據才形成刑事疑義完整方向',async()=>{
  ...
  assert.match(f.waterArticle35Text,/構成要件事實已完整/);
...
test('4.0-rc1 §26需先建立合法查證基礎再判規避妨礙拒絕',async()=>{
  ...
  assert.match(f.waterArticle26Text,/構成要件事實已完整/);
});
...
test('4.0-rc1 最終輸出在無成立方向且查證未完成時為C，完成時可為A',async()=>{
  ...
  assert.match(pending.waterFinalConclusionText,/C｜尚有要件待確認/);
  ...
  assert.match(clear.waterFinalConclusionText,/A｜目前不支持/);
});
