(function(root){
  ...
  const t={
    ...
    fields:[
      ...
      field('noiseSubject','稽查對象／場所','text',{displayWhen:when('noiseShowMeasure')}),field('noiseSource','主要噪音源／設備說明','text',{showWhen:when('noiseShowMeasure'),displayWhen:{field:'noiseA9Type',value:'speaker',operator:'notEquals'}}),
      ...
      field('noiseWind','室外量測風速（m/s）','text',{inputMode:'decimal',mobileKeepVisible:true,displayWhen:when('noiseShowWeather')}),
      field('noiseValueFull','測定均能音量 Leq dB(A)','text',{inputMode:'decimal',mobileKeepVisible:true,displayWhen:when('noiseShowFullAssessment')}),
      field('noiseValueLeq','營建／公告工程全頻 Leq dB(A)','text',{inputMode:'decimal',mobileKeepVisible:true,displayWhen:when('noiseShowConstructionFull')}),
      field('noiseValueLmax','營建／公告工程全頻 Lmax dB(A)','text',{inputMode:'decimal',mobileKeepVisible:true,displayWhen:when('noiseShowConstructionLmax')}),
      field('noiseValueLow','低頻 Leq,LF dB(A)','text',{inputMode:'decimal',mobileKeepVisible:true,displayWhen:when('noiseShowLow')}),
      select('noiseBgFullMode','全頻背景音量處理',[['measured','已量測背景音量'],['uncooperative','負責人／現場人員無法配合，依法不修正並註明'],['pending','尚未完成背景音量確認']],{mobileKeepVisible:true,displayWhen:when('noiseShowBgFull')}),
      field('noiseBgFull','全頻背景音量 dB(A)','text',{inputMode:'decimal',mobileKeepVisible:true,displayWhen:{field:'noiseBgFullMode',value:'measured'}}),
      select('noiseBgLmaxMode','Lmax 背景音量處理',[['measured','已量測背景音量'],['uncooperative','負責人／現場人員無法配合，依法不修正並註明'],['pending','尚未完成背景音量確認']],{mobileKeepVisible:true,displayWhen:when('noiseShowBgLmax')}),
      field('noiseBgLmax','Lmax 背景音量 dB(A)','text',{inputMode:'decimal',mobileKeepVisible:true,displayWhen:{field:'noiseBgLmaxMode',value:'measured'}}),
      select('noiseBgLowMode','低頻背景音量處理',[['measured','已量測背景音量'],['uncooperative','負責人／現場人員無法配合，依法不修正並註明'],['pending','尚未完成背景音量確認']],{mobileKeepVisible:true,displayWhen:when('noiseShowBgLow')}),
      field('noiseBgLow','低頻背景音量 Leq,LF dB(A)','text',{inputMode:'decimal',mobileKeepVisible:true,displayWhen:{field:'noiseBgLowMode',value:'measured'}}),
