(function(root){
  ...
  function sourceLabel(input){
    if(input.noiseSourceCategory==='equipment'){
      const row=rules.equipment.find(x=>x.id===input.noiseEquipmentType);
      return text(input.noiseSourceDescription)||row?.label||'機械／設備';
    }
  ...
  function targetTable(target){
    if(!target)return '';
    return {factory:'factory',business:'business',construction:'construction',renovation:'construction',speaker:'speaker',otherFacility:'other'}[target.id]||'';
  }
  ...
  function a8State(input,zones,evidence,out){
    ...
    if(!actId)return {status:'none',text:'第8條：依目前已填現場行為，未形成第8條公告禁止行為候選。'};
    const act=(root.NOISE_ARTICLE8_RULES?.acts||[]).find(x=>x.id===actId);
    if(!act)return {status:'pending',text:'第8條：現場行為已有紀錄，但對應公告規則尚未載入。'};
    if(!text(input.noiseDate)||!timeValid(input.noiseTime)||!zones.length)return {status:'pending',act,text:'第8條：待補稽查日期、時間及管制區後研判公告時段。'};
    const holiday=holidayState(input);
    if(!holiday)return {status:'pending',act,text:'第8條：例假日／上班日狀態尚待確認。'};
  ...
  function draftA9(input,target,assessment,zones){
    ...
    const weather=outdoor&&text(input.noiseWeatherText)?'量測時天候'+text(input.noiseWeatherText)+(num(input.noiseWind)!==null?'，風速為'+fmt(input.noiseWind)+'m/s，':'，'):'';
    const mixed=[];
    ...
    let ending='';
    if(statuses.includes('high'))ending='經比對本案適用之噪音管制標準，'+result+'，依噪音管制法第9條規定辦理限期改善，經限期改善仍未符合噪音管制標準者將依法告發。';
    else if(statuses.includes('unable'))ending=result+'。';
    else ending='經比對本案適用之噪音管制標準，'+result+'。';
    return '本局於'+rocDate(input.noiseDate)+inspectionTime(input.noiseTime)+'派員前往所陳地點，經查該址為'+text(input.noiseSubject)+'，稽查時作業中，噪音源為'+source+'，'+weather+'於'+location+(text(input.noiseMeasurementPlaceDetail)?'（'+text(input.noiseMeasurementPlaceDetail)+'）':'')+'量測'+parts.join('、')+(bg?'，'+bg:'')+'。'+ending;
  }
  function pendingItems(input,target,zone,assessment,a8){
    ...
    if(input.noiseTargetRunning==='yes'){
      ...
      if(selected(input.noiseMeasureBands,'full')){
        ...
        if(input.noiseMeasurementPlace==='boundary'&&(!text(input.noiseWeatherText)||num(input.noiseWind)===null))items.push('室外天候／風速');
        if(!assessment.full?.status&&assessment.full?.progress)items.push('全頻判定所需資料');
      }
      if(selected(input.noiseMeasureBands,'low')){
        if(!text(input.noiseLowStart)||!text(input.noiseLowEnd))items.push('低頻量測開始／結束時間');
        if(!assessment.low?.status&&assessment.low?.progress)items.push('低頻判定所需資料');
      }
      if(!input.noiseBehavior)items.push('現場行為');
    }
  ...
  function prepare(raw={}){
    ...
    if(!input.noisePlaceType)return setStage(out,'場所／工程','請依現場實際情形選擇場所／工程類型。','請確認場所／工程。');
    out.noise522ShowPlaceLegal=yes(input.noisePlaceType==='other');
    if(input.noisePlaceType==='other'&&!input.noisePlaceLegalClass)return setStage(out,'場所法律性質','「其他」場所不由系統猜測，請補選法律場所性質或保持尚待確認。','請確認其他場所之法律性質。');

    out.noise522ShowSource='yes';
    if(!input.noiseSourceCategory)return setStage(out,'主要噪音來源','請確認主要噪音來源；場所、音源與現場行為分開記錄。','請確認主要噪音來源。');
    out.noise522ShowEquipment=yes(input.noiseSourceCategory==='equipment');
    out.noise522ShowSourceOther=yes(input.noiseSourceCategory==='other');
    if(input.noiseSourceCategory==='equipment'&&!input.noiseEquipmentType)return setStage(out,'機械／設備種類','請確認主要機械／設備種類。','請確認機械／設備種類。');
    ...
    out.noise522ShowTargetManual=yes(candidates.length===0);
    ...
    out.noise522ShowZoneSimple=yes(input.noiseBoundaryInvolved==='no');
    ...
    if((target.id==='factory'||target.id==='business'||target.id==='otherFacility')&&!selected(input.noiseGeneralSpecialAssessment,'periodic'))forwarded.noiseGeneralMethod='leq';
    if(target.id==='construction'||target.id==='renovation'){
  ...
  function resetChange(before={},after={}){
    ...
    if(before.noiseContinuity!==after.noiseContinuity||before.noiseMeasurability!==after.noiseMeasurability){
      if(after.noiseContinuity!=='yes'||after.noiseMeasurability!=='yes')clear(['noisePlaceType','noisePlaceLegalClass','noiseSourceCategory','noiseEquipmentType','noiseTargetChoice','noiseTargetManual','noiseTargetRunning','noiseMeasureBands']);
    }
    if(before.noisePlaceType!==after.noisePlaceType||before.noisePlaceLegalClass!==after.noisePlaceLegalClass){
    ...
    if(before.noiseMeasurementPlace!==after.noiseMeasurementPlace)clear(['noiseWind']);
  ...
  root.TemplatePatches.noiseRefactor522=config=>{
    ...
    const select=(id,label,options,displayWhen)=>field(id,label,'select',{allowCustom:false,options:options.map(x=>({id:x.id||x[0],label:x.label||x[1],value:x.label||x[1]})),...(displayWhen?{displayWhen}:{})});
    ...
    const wind=t.fields.find(f=>f.id==='noiseWind');if(wind)wind.displayWhen=show('noise522ShowWeather');
    ...
    const newFields=[
      ...
      select('noisePlaceType','場所／工程',rules.places,show('noise522ShowPlace')),
      field('noisePlaceOtherText','其他場所說明','text',{displayWhen:{field:'noisePlaceType',value:'other'}}),
      select('noisePlaceLegalClass','其他場所之法律性質',rules.placeLegalClasses,show('noise522ShowPlaceLegal')),
      select('noiseSourceCategory','主要噪音來源',rules.sources,show('noise522ShowSource')),
      select('noiseEquipmentType','機械／設備種類',rules.equipment,show('noise522ShowEquipment')),
      ...
      field('noiseFullStart','全頻量測開始時間','time',{displayWhen:show('noise522ShowFull')}),
      field('noiseFullEnd','全頻量測結束時間','time',{displayWhen:show('noise522ShowFull')}),
    ...
    const existingIds=new Set(newFields.map(f=>f.id));
    ...
    t.formTitle='噪音案件';
    t.version='5.2.2-test.1';
    t.moduleVersion='5.2.2-test.1';
