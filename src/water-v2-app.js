(function(root){
  ...
  function syncIncidents(state){
    ...
    if(value(state,'water.treatment.operating')==='not_operating'||value(state,'water.treatment.abnormal_operation')==='yes')ensureIncident(state,'treatment_nonoperation','處理設施未正常操作','high','inspect_treatment');
  ...
  function panel(title,cls=''){const s=E('section',null,'water-v2-panel '+cls);if(title)s.append(E('h3',title));return s;}
  ...
  function renderW05(ctx,state){
    const p=panel('W05｜確認場所與稽查對象');p.append(note('「附近有工廠」不等於來源已確認。場所關聯與實際營運主體分開記錄。'));
    p.append(factSelect(ctx,state,'water.premises.relation_status','場所與水流關聯',[['candidate','候選'],['nearby','僅位置接近'],['supported_relation','有支持關聯事實'],['confirmed_relation','已確認關聯'],['excluded','已排除'],['unresolved','仍無法確認']]));
    p.append(factSelect(ctx,state,'water.subject.type','主體類型',[['business','水污法事業'],['sewer_system','污水下水道系統'],['building_sewage_facility','建築物污水處理設施'],['individual','一般行為人'],['other','其他'],['unknown','尚未確認']],{category:'record'}));
    let sub=state.subjects[0];if(!sub){sub={id:'S001',roles:[],name:'',identityStatus:'unknown'};state.subjects.push(sub);}p.append(row('現場實際營運／行為人名稱',inputControl('text',sub.name||'','可用代稱',v=>{sub.name=v;persist(ctx,state);})));p.append(row('身分確認狀態',selectControl([['confirmed','已確認'],['likely','高度可能'],['conflict','資料衝突'],['unknown','尚未確認']],sub.identityStatus||'unknown',v=>{sub.identityStatus=v;setFact(state,'water.subject.identity_status',v,{category:'record'});setFact(state,'water.subject.identity_confirmed',v==='confirmed'?'yes':v==='unknown'?'unknown':'no',{category:'record'});persist(ctx,state);})));return p;
  }
  function renderW06(ctx,state){
