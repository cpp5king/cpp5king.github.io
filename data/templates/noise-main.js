(function(root){
  ...
  const t={
    ...
    version:'4.9-rebuild-5',moduleVersion:'4.9-rebuild-5',workflow:'noiseMain',choiceStyle:'cards',initialGate:true,
    ...
    fields:[
      ...
      field('noiseDate','第一步｜稽查日期','date',{format:'roc'}),field('noiseTime','稽查時間','time'),
      select('noiseZoneMode','噪音管制區判定方式',[['direct','已知管制區，直接選擇'],['assist','依新北市115年3月5日公告協助判定']]),
      select('noiseZone','噪音管制區（已知時直接選擇）',zones),
      ...
      select('noiseZoneAdjacentFirst','道路／運輸設施是否緊鄰第一類管制區',ynu,{displayWhen:{field:'noiseZoneAssistType',value:'majorTransport'}}),
      ...
      select('noiseNature','第四步｜一般噪音源是否具持續性且可量測？',[['difficult','不具持續性或不易量測'],['measurable','具持續性，可依噪音量測程序確認']],{displayWhen:{field:'noiseSpecial',value:'ordinary'}}),
