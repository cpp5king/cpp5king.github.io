(function(root){
  ...
  const t={
    ...
    version:'4.9-rebuild-1',moduleVersion:'4.9-rebuild-1',workflow:'noiseMain',choiceStyle:'cards',initialGate:true,
    ...
    fields:[
      computed('noiseShowOrdinary','ordinary'),computed('noiseShowA8','a8'),computed('noiseShowA8Disturbance','a8d'),
      computed('noiseShowA8Exception','a8e'),computed('noiseShowA9','a9'),computed('noiseShowOtherFacility','other'),
      computed('noiseShowMeasure','measure'),computed('noiseShowFull','full'),computed('noiseShowLow','low'),
      computed('noiseShowConstructionLmax','lmax'),computed('noiseShowBgFull','bgfull'),computed('noiseShowBgLmax','bglmax'),computed('noiseShowBgLow','bglow'),
      computed('noiseBlocked','blocked'),computed('noiseValidation','validation'),computed('noiseRecord','record'),computed('noiseReply','reply'),
      select('noiseNature','第一步｜聲音是否具持續性且可量測？',[
        ['difficult','不具持續性或不易量測'],['measurable','具持續性，可依噪音量測程序確認']
      ]),
      select('noiseSpecial','第二步｜主要噪音來源',[
      ...
      ],{displayWhen:when('noiseNature','measurable')}),
      field('noiseDate','稽查日期','date',{format:'roc',displayWhen:when('noiseShowOrdinary')}),
      field('noiseTime','稽查時間','time',{displayWhen:when('noiseShowOrdinary')}),
      select('noiseZone','噪音管制區',zones,{displayWhen:when('noiseShowOrdinary')}),
      select('noiseHoliday','是否為例假日／國定假日',yn,{displayWhen:when('noiseShowOrdinary')}),
      select('noiseA8Act','第三步｜現場行為是否屬新北市第8條公告禁止行為',acts,{displayWhen:when('noiseShowA8')}),
      ...
      select('noiseA9Type','第四步｜第9條噪音源類型',[
      ...
      ],{displayWhen:when('noiseShowFull')}),
      select('noiseFullIndoor','全頻測點是否位於室內',yn,{displayWhen:{field:'noiseFullPoint',value:'complainant'}}),
      select('noiseRain','室外量測時是否下雨',yn,{displayWhen:when('noiseShowFull')}),
      field('noiseWind','室外量測風速（m/s）','text',{inputMode:'decimal',displayWhen:when('noiseShowFull')}),
