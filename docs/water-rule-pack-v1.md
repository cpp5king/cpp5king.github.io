...
新增 `tests/water-scenario-acceptance.test.cjs`，固定八類現場情境作為上線前驗收基準：

1. 來源不明：污染點 → 追水 → 疑似來源 → 確認來源 → 對象查核；疑似來源階段不得提前產生法規方向。
2. 已知事業＋有效排放許可＋A～F無異常：不得把正常狀態誤判為違規方向。
3. 無有效排放許可＋地面水體排放：可列第14條可能方向，但不得輸出「違規成立」。
4. 有效排放許可但現場／許可不一致：許可差異與非核准出口分流，無貯留／稀釋事實時不得混入第20條。
5. 污水下水道系統：第14、18條依第19條準用；第20條維持直接適用，不錯冠第19條。
6. 特定業別：半導體第49條之9分流欄位、Rule Pack 法規內容與版本正常顯示。
7. 行為日期不明：不得以稽查日期或目前日期代替；具體法規方向暫不輸出，版本維持待確認。
8. 重新研判：第一次事實、結果、Rule Pack 版本與輸出快照保留；第二次新增 review，不覆寫第一次。

首次執行僅有一項驗收文字不一致：程式輸出「已確認符合」，測試原預期「已完成」。此為測試用語過度概括，保留較精確的程式文字並修正驗收條件；未發現流程性回歸。
